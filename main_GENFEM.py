...
loconame_genfem_stand = "generic_average_female_locomotion.anims"
loconame_genfem_crouch = "wa_gang_unarmed_locomotion_stealth.anims"

prefix_genfem_folder_stand = "0_genfemPOSE_S_"
prefix_genfem_folder_crouch = "0_genfemPOSE_C_"
...
def packer():
    ...
    try:
        for archive_path in archives:
            ...
            if "\\" + prefix_genfem_folder_crouch in archive_path:
...
def locomotion_convertor():
    global total_conversions, prefix_genfem_folder_stand, prefix_genfem_folder_crouch
    for folder in os.listdir(dir_output_unbundled):
        ...
        # Build target NPC folders
        genfem_folder_stand = os.getcwd() + "\\tempfiles\\" + prefix_genfem_folder_stand + folder + "_WOLV"
        genfem_folder_crouch = os.getcwd() + "\\tempfiles\\" + prefix_genfem_folder_crouch + folder + "_WOLV"
        npc_folder_builder(genfem_folder_stand)
        npc_folder_builder(genfem_folder_crouch)

        # Destination paths where we'll store the locomotions
        loc_path_stand = "\\base\\animations\\npc\\generic_characters\\female_average\\locomotion"
        ...
        # Copy anim to target NPC folders and rename
        anim_copy(path_to_anim, genfem_folder_stand +
                  loc_path_stand, loconame_genfem_stand, source_locomotion_file)
        anim_copy(path_to_anim, genfem_folder_crouch +
